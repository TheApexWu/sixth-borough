...
Table of Contents
TL;DR (30 seconds)
The 4 Pillars (Core Identity)
How The Pillars Synthesize
The Demo We're Shipping
Scope: IN vs OUT
The Stack (pinned)
Repo Skeleton
Roles (Alex / Carson / James / Marvens)
Risks + Mitigations
Dual Pitch (NVIDIA vs Antler)
...
(0:00 – 0:30) Team intro. Four names, four roles, one sentence each. Alex (orchestration + narration), Carson (Rust + wgpu engine), Marvens (3D / point cloud / visual identity), James (cultural curation + WebGL fallback).
(0:30 – 1:00) Elevator pitch. "Sixth Borough is a time machine for niche subcultures. We start with hip-hop in the Bronx — the most-mythologized scene in NYC, mapped to coordinates for the first time, rendered in the visual language of the era it belongs to. Everything runs on a single DGX Spark with no internet."
(1:00 – 2:30) Live demo.
...
☐ Demo video uploaded to YouTube (unlisted), 3–5 min, contains all the Spark Story words above
☐ Repo public on GitHub with README updated, architecture diagram, quick start, datasets cited, known limitations
☐ Airtable form filled out with: team name, project description, Cultural Impact track selected, video URL, repo URL, deployed URL (or screen capture), team roster (4 names: Alex / Carson / James / Marvens)
☐ Backup: at least one team member has the Airtable receipt screenshot
☐ Pitch script for the in-person Hack Fair table rehearsed once
☐ Hardware ready to walk to the Hack Fair station
